...
const CleanGraphs = ({ data, region, chosenRegion }) => {
    ...
    function createSeries(array) {
        ...
        if (options.value === "NoSortNeeded") {
            ...
            */
            const newArray = array[0].properties.verdier.filter((e) =>
                JSON.stringify(e.filters) === JSON.stringify(Object.values(sorting.options[sorting.id]))
            )
    ...
    function createMultiSeries(dataArray) {
        ...
        if (options.value === "NoSortNeeded") {
            ...
            series = dataArray.map(p => {
                const testValue = p.properties.verdier.filter((e) =>
                    JSON.stringify(e.filters) === JSON.stringify(Object.values(sorting.options[sorting.id]))
                )
                const currData = Object.values(testValue[0][options.ContentsCodes[sorting.contentCodeIndex].label])
                return { name: p.properties.Region,data:currData}
            })
        }
        return series
        //dataArray.filter()
